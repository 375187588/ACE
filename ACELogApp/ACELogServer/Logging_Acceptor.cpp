...
int Logging_Acceptor::handle_input(ACE_HANDLE)
{
  ...
  {
    delete peer_handler; 
	std::cout << "Logging_Acceptor acceptor_.accept" << std::endl;
    return -1; 
  }
  else if(peer_handler->open() == -1)
  {
    peer_handler->handle_close(); 
	std::cout << "Logging_Acceptor acceptor_.open" << std::endl;
    return -1; 
  }

  std::cout << "Logging_Acceptor handle_input" << std::endl;
...
int Logging_Acceptor::handle_close(ACE_HANDLE, ACE_Reactor_Mask)
{
  acceptor_.close(); 
  delete this; 
  std::cout << "Logging_Acceptor acceptor_.handle_close" << std::endl;
  return 0; 
}
